...
$(function () {
    ...
    function initTabSwipers() {
        $(".btns-swiper-group").each(function (index) {
            const $container = $(this);
            
            // 각 탭 스와이프 컨테이너에 클래스를 추가합니다.
            $container.addClass(`btns-swiper-group-${index}`);

            // Swiper 라이브러리를 사용하여 탭 스와이프를 초기화합니다.
            const swiper = new Swiper(`.btns-swiper-group-${index}`, {
                slidesPerView: "auto",
                preventClicks: true,
            ...
            $container.on('click', '.swiper-slide .btn-swiper-tab', function (e) {
                e.preventDefault();
    ...
    const detailGoodsSwiper = new Swiper('#detailGoodsSwiper', {
        ...
        slidesPerView: 1,
        pagination: {
            el: ".detail-goods-swiper > .swiper-pagination",
            clickable: true,
            watchOverflow: true
        },
        navigation: {
            nextEl: ".detail-goods-swiper > .swiper-button-next",
            prevEl: ".detail-goods-swiper > .swiper-button-prev"
        }
    });
